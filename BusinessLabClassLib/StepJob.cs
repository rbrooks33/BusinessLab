    ...
    public class StepJob : IJob
    {
        ...
        Task IJob.Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                Actions.SendJobTraceMessage($"Executing Action #{name}.");
                ...
                if (action.ActionID > 0)
                {
                    ...
                    {
						//Run fail action
						Actions.SendJobTraceMessage($"Failed executing Action #{name}. Checking for Fail Action...");

                        if (action.FailActionID > 0)
                        {
                            Actions.SendJobTraceMessage($"Found action #{ action.FailActionID.ToString()}");
                            ...
                            Actions.SendJobTraceMessage($"No fail action for #{action.ActionID}");
                    }
                    else
                    {
                        //Run success action
                        Actions.SendJobTraceMessage($"Success running Action #{name}! Checking for Success Action...");
                        
                        if (action.SuccessActionID > 0)
                        {
							Actions.SendJobTraceMessage($"Found action #{action.SuccessActionID.ToString()}");
            ...
            catch (Exception ex)
            {
				...
				Actions.SendJobTraceMessage(failMessage);
